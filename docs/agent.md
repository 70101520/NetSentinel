...
The implemented Windows Service foundation lives in `agent/windows`. Its current scope is enrollment and heartbeat only. It uses a random persisted installation UUID, DPAPI machine-scope credential blobs protected by ProgramData ACLs, LocalService, cancellable scheduling, bounded jittered backoff, rotating logs, safe status output, and explicit HTTP test-only opt-in. See that directory's README for installation, recovery, security, and validation procedures.
