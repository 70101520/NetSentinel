...
Run `scripts/bootstrap-primary-discovery.sh` on the Linux Docker host after deployment. It derives the canonical RFC1918 subnet from the host's default-route interface and idempotently registers it as `Primary server network`; no credential or Docker socket is exposed to the API container. Additional routed VLANs remain explicit administrator entries.
