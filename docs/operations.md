...
Compose marks the API healthy only after migrations finish and `/health/ready` confirms PostgreSQL and Redis. The portal waits for that health state before starting, preventing transient reverse-proxy 502 responses during normal deployments.
